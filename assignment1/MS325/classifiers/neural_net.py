...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # shape (N, C).                                                             #
    #############################################################################
    h_output = np.zeros_like(np.dot(X, W1) + b1)
    h_output = np.maximum(np.dot(X, W1) + b1, 0) 
    ...
    # classifier loss.                                                          #
    #############################################################################
    #shift_scores = np.exp(scores - np.reshape(np.max(scores, axis = 1), (N, 1)))
    ...
    dscores /= N
    grads['W2'] = np.dot(h_output.T, dscores) + reg * W2
    ...
    #dh[h_output <= 0] = 0
    #dh_ReLu = 0
    #grads['W1'] = reg * W1 + np.dot(X.T, dh)
    #grads['b1'] = np.sum(dh, axis = 0, keepdims = True)
    dh = np.zeros_like(h_output)
    dh[h_output > 0] = 1
    dh_ReLu = np.dot(dscores, W2.T)*dh
    grads['W1'] = np.dot(X.T, dh_ReLu) + reg * W1
    grads['b1'] = np.ones(N).dot(dh_ReLu)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
