...
def svm_loss_vectorized(W, X, y, reg):
  ...
  correct_scores = np.reshape(correct_scores, (X.shape[0], 1))
  margin = scores - correct_scores + 1.0
  margin[np.arange(X.shape[0]),list(y)] = 0.0
  ...
  gra = margin
  gra[margin>0] = 1.0
  gra[range(X.shape[0]),list(y)] = -np.sum(gra, axis = 1)
  dW = dW + np.dot(X.T, gra)/X.shape[0] + reg * W
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
